/**
 * MeshCentral Mesh Branding v2.0.0
 * Branding global por subdominio.
        ...
        (function() {
            ...
            function ensureStyle() {
                ...
                style.textContent = '' +
                  ...
                  'img[data-meshbranding-replaced="1"]{object-fit:contain!important;}';
            ...
            function looksLikeBrandingImage(img) {
                ...
                if (text.indexOf('masthead') >= 0 && text.indexOf('logo') >= 0) return true;
                if (text.indexOf('/logo') >= 0 || text.indexOf('logo.') >= 0 || text.indexOf('-logo') >= 0 || text.indexOf('_logo') >= 0) {
                    if (text.indexOf('icon-') >= 0 || text.indexOf('x16') >= 0 || text.indexOf('plugin24') >= 0) return false;
                    return true;
                }
                return false;
            }
            function replaceBrandingImages(brand, embedded) {
                if (!embedded.logoDataUri) return;
                var imgs = document.querySelectorAll('img');
                for (var i=0;i<imgs.length;i++) {
                    var img = imgs[i];
                    if (!looksLikeBrandingImage(img)) continue;
                    var old = img.getAttribute('src') || '';
                    if (old === embedded.logoDataUri) continue;
                    if (!img.getAttribute('data-meshbranding-original-src')) img.setAttribute('data-meshbranding-original-src', old);
                    img.src = embedded.logoDataUri;
                    img.alt = brand.documentTitle || img.alt || 'MeshCentral';
                    img.setAttribute('data-meshbranding-replaced','1');
                    if (img.id === 'MainMeshImage') { img.style.maxHeight = img.style.maxHeight || '86px'; img.style.width = img.style.width || '260px'; img.style.height = 'auto'; }
                }
            }
            function replaceBrandingBackgrounds(brand, embedded) {
                ...
                for (var i=0;i<nodes.length;i++) {
                    ...
                    if (bgl.indexOf('icon-') >= 0 || bgl.indexOf('x16') >= 0) continue;
            ...
            function applyBranding() {
                ...
                if (opt.replaceBrandingImages !== false) replaceBrandingImages(brand, embedded);
                if (opt.replaceBrandingBackgrounds !== false) replaceBrandingBackgrounds(brand, embedded);
                // Intencionalmente NAO altera textos internos como #p6title h1: preserva "Meu Servidor".
                log('aplicado', resolved.host);
            }
